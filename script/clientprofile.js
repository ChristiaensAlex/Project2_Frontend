let firstname, lastname, username, password, passwordRepeat, extra, profilepic;
...
const putClientInfoAPI = function(url, payload) {
  ...
  fetch(url, {
    ...
    .then(res => {
      console.log(res.status)((window.location.href = 'https://trekjeplan-front.azurewebsites.net/MentorHasClientList.html'));
    })
    // .then(data => {
    // 	console.log(data); // ;
    // })
...
const showClientInfo = function(json) {
  ...
  passwordConfirm.value = json.password;
...
const GetDomElementsClient = function() {
  ...
  passwordRepeat = document.querySelector('.js-password-repeat');
